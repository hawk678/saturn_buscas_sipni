...
async function getInfoVacinas2(req, res) {
...
const imunobiologicos = data.record.outrasImunizacoes.imunobiologicos;

const vacinas = [];

for (const imunização of imunobiologicos) {
  const imunizacoes = imunização.imunizacoes;
  for (const imunizacao of imunizacoes) {
    const vacinaCovid = {
      sigla: imunização.sigla,
      abreviatura: imunização.abreviatura,
      nome: imunização.nome,
      ...
      local: {
        ...
        sistemaOrigem: imunizacao.sistemaOrigem
      }
    };
    vacinas.push(vacinaCovid);
  }
}
...
module.exports = { getInfoVacinas2 }